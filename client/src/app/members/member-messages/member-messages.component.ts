...
export class MemberMessagesComponent implements OnInit, AfterViewChecked {
  @ViewChild('scroller') private messageFeed: ElementRef;
  ...
  sendMessage() {
    this.loading = true;
    this.messageService.sendMessage(this.username, this.messageContent).then(() => this.messageForm.reset()).finally(() => this.loading = false);
  }  
}
